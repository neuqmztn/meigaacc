...
// 1. 混合精度数据流 (8-16-8-16)：
//    - 输入/权重 (8-bit): 标准 BFP8 格式，节省外部存储和带宽。
//    - Linear1 输出 (16-bit): 扩展精度，保留梯度信息给 GELU。
//    - GELU 输出 (8-bit): 内部高精度计算后压缩回 8-bit，减半中间 Buffer 需求。
//    - H_act Buffer (8-bit): 单端口 SRAM，存储压缩后的激活值。
//    - Linear2 输出 (16-bit): 恢复高精度进行累加，防止溢出。
//    - 最终输出 (8-bit): 截断回 8-bit 存入外部 Result Buffer。
//
// 2. 全链路独立指数支持：
//    - 32 个 Token 的指数在 Linear1 生成，透传过 GELU。
//    - 通过专门的打包/解包状态机，将并行的 32 个指数串行写入/读取单端口 Buffer。
//    - Linear2 和累加器均支持处理这 32 个独立的指数。
//================================================================================
...
module ffn_backbone_top #(
    ...
    parameter ADDR_WIDTH     = 10,     // 外部 Buffer 地址位宽
        ...
        if(!rst_n) begin
            ...
            case(token_load_state)
                ...
                3'd3: begin // RECEIVE DATA
                    ...
                    if(token_load_count == 0) begin
                        token_block_exp <= rb_rd_exp;
                    end
                    // 存储尾数
    ...
    linear_compute_engine #(
        .TOKEN_CHUNK(TOKEN_CHUNK), .INPUT_DIM(D_MODEL), .OUTPUT_DIM(FEATURE_CHUNK),
        .BFP_EXP_W(BFP_EXP_W),
    ...
    gelu_engine #(
        .TOKEN_CHUNK(TOKEN_CHUNK), .DIM(FEATURE_CHUNK),
        .BFP_EXP_W(BFP_EXP_W),
