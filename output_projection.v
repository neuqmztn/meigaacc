...
//================================================================================
// Output Projection - BFP版本 v2.1（修复BFP转换器接口 + CE结果锁存 + 更鲁棒的Head读取）
//
// 变更摘要（v2.1）
//   - BFP转换器实例化参数名/端口名改为：TOTAL_RESULTS/FIXED_WIDTH/BASE_EXP_WIDTH
//   - CE结果在送入BFP转换器前进行锁存，转换阶段保持稳定
//   - Head读取等待逻辑由"同时到齐"改为"位掩码收集"，适配不同head的变长延迟
//   - 保留24bit累加精度，在送入CE前做 24→8bit 的BFP下采样（整token共享指数）
//
// 日期：2024-11-15
//================================================================================
...
// 24→8 bit 下采样（保持 per-token 共享指数）
...
begin
    shifted = in_val >>> DOWN_SHIFT; // 算术右移保留符号
...
compute_engine #(
    ...
    .exp_X       (concat_exp_q),
    .mant_X_block(concat_mant_q_packed),
...
//------------------------------------------------------------------------------
// BFP 转换器（使用你当前工程的参数/端口名）
//   TOTAL_RESULTS = DIM
//   FIXED_WIDTH   = CE_OUTPUT_WIDTH
//   BASE_EXP_WIDTH= CE_BASE_EXP_WIDTH (= EXP_WIDTH+1)
//------------------------------------------------------------------------------
fixed_to_independent_bfp #(
    ...
    .input_valids     (ce_out_valids_r),      // ✅ 来自锁存
    .input_fixed_array(ce_out_fixed_r),      // ✅ 来自锁存
    .input_base_exp_array  (ce_out_base_exp_r),    // ✅ 来自锁存
    .input_zero_array      (ce_out_zero_r),        // ✅ 来自锁存

    .output_valids    (bfp_valids),
    .output_mant_array     (bfp_mants),
    .output_exp_array(bfp_shared_exp),
    .output_overflow  (bfp_overflow)
);

//------------------------------------------------------------------------------
// 主状态机（含更鲁棒的Head读取/CE结果锁存）
//------------------------------------------------------------------------------
integer h, d;
reg [3:0] read_done_mask; // 每维度读取完成掩码（bit0..bit3 对应 h0..h3）
        ...
        case (state)
            ...
            STATE_READ_HEADS: begin
                if (dim_idx < HEAD_DIM) begin
                    ...
                    read_done_mask    <= 4'b0000; // 新一维开始收集
                    read_wait_counter <= 4'd0;
            ...
            STATE_WAIT_READ: begin
                ...
                if (&read_done_mask) begin
                    ...
                    if (read_wait_counter > 4'd15) begin
                        // 超时复位本次token处理，返回IDLE（可改为更温和的恢复策略）
                        state <= STATE_IDLE;
                    end
                end
            end

            // 指数对齐 + Concat（组合逻辑完成，单拍过渡）
            ...
            // 等待 CE 完成，同时在完成时锁存 CE 输出，供 BFP 转换器使用
            STATE_WAIT_CE: begin
                ce_input_valid <= 1'b0;
...
initial begin
    $display("========================================");
    $display("Output Projection v2.1 Initialized");
    $display("  Heads: %0d x Dims: %0d = DIM: %0d", NUM_HEADS, HEAD_DIM, DIM);
    $display("  Accum mant width: %0d, CE input mant: %0d", ACCUM_WIDTH, DATA_WIDTH);
    $display("  BFP downcast: shared exp shift = %0d", DOWN_SHIFT);
    $display("  Robust read: per-head valid mask collection");
    $display("  CE outputs latched before BFP converter");
    $display("========================================");
end
`endif
