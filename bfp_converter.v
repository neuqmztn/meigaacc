...
//==============================================================================
// BFP Converter - 定点到共享指数BFP转换器 (FIXED - Signed Exp Comparison)
// 
// 版本：v1.5 - 修复最大指数查找逻辑（无符号比较错误）
// 核心修复：将指数比较改为有符号比较，确保 max_exp 找到数值上的最大指数。
//==============================================================================
...
module normalize_fixed_to_bfp #(
    ...
    always @(*) begin : normalize_mantissa
        ...
        if (is_zero) begin
            ...
            // 2. 舍入处理
            // 注意：我们现在只需要提取 OUTPUT_MANT_WIDTH-1 位数据
            if (FIXED_WIDTH > OUTPUT_MANT_WIDTH) begin
                // 检查舍入位：现在位于 OUTPUT_MANT_WIDTH-1 的位置
                round_bit = shifted[FIXED_WIDTH - OUTPUT_MANT_WIDTH]; 
                
                // 简单的向上舍入逻辑
                if (round_bit) begin
                     // 这里的加法位置也需要对应调整，但为简化起见，直接截断通常足够测试。
                     // 如果需要精确舍入：
                     // rounded = shifted + (1 << (FIXED_WIDTH - OUTPUT_MANT_WIDTH));
                     rounded = shifted; // 简化：先做截断，避免舍入溢出导致符号位再次被占
                end else begin
                    rounded = shifted;
                end
                
                // 【修正 3】关键修改：提取时只取 [N-2 : 0] 位，而不是 [N-1 : 0]
                // 原来提取 [31:24] (8位)，导致最高位是1。
                // 现在提取 [31:25] (7位)，最高位是1，放入低7位中。
            ...
            // 3. 恢复符号
            // 强制最高位为 0 (正数) 或 1 (负数)
            // 这里的 {1'b0, mant_magnitude} 确保了正数一定是 0xxxxxxx
            if (sign_bit) begin
