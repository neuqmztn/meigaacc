`timescale 1ns / 1ps
// ============================================================================
// DFA Training Subsystem v2
//
// 新架构：
//   - 单 GCU：dfa_gcu_training_core_v2 + gcu_core_v2
//   - 单梯度缓存：gradient_buffer_v2
//   - 单 B 矩阵 BRAM：dfa_matrix_bank_v2
//   - 单权重更新引擎：weight_update_engine
//
// 不再支持：
//   - GCU0..4
//   - 旧 gradient_buffer 多端口
//   - 旧 dfa_matrix_bank 多端口
//   - expand(layer4) DFA 训练（已冻结）
//
// 顶层工作流程：
//   1. 前向推理结束后，上层给 train_start = 1
//   2. training_core 串行跑 layer0..3 → 产生梯度 → 写入 gradient_buffer_v2
//   3. training_core done_all = 1 → 进入权重更新阶段
//   4. weight_update_engine 读梯度 → 更新权重
//   5. weight_update_done = 1 → train_done = 1
// ============================================================================
