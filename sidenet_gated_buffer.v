...
initial begin
    $display("========================================");
    $display("Sidenet Gated Buffer (Optimized)");
    $display("========================================");
...
always @(posedge clk) begin
    if (wr_en && wr_addr_valid) begin
        $display("[%0t] Gated Buffer WRITE: addr=%0d, exp=0x%h, mant=0x%h", 
...
always @(posedge clk) begin
    if (rd_en && rd_addr_valid) begin
        $display("[%0t] Gated Buffer READ:  addr=%0d", $time, rd_addr);
    end
end
`endif
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (cycle_count % 32'd10000 == 32'd0 && cycle_count != 32'd0) begin
            $display("----------------------------------------");
            $display("[%0t] Gated Buffer Statistics:", $time);
...
// 属性4：门控数据完整性（非零数据写入后可读）
property p_gated_data_integrity;
    @(posedge clk) disable iff (!rst_n)
    (wr_en && wr_addr_valid && (wr_mant != 0)) |-> 
    ##1 (mant_mem[$past(wr_addr)] == $past(wr_mant));
endproperty
assert property (p_gated_data_integrity);
`endif
