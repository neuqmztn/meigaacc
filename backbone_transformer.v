...
//================================================================================
// Transformer Block - 单层处理模块（不含层间存储）
//
// 版本：v2.2
// 日期：2024-11-16
//
// 更新内容（v2.1 → v2.2）：
// - 删除FFN DRAM接口
// - 添加FFN权重请求接口（连接到weight_controller）
// - 统一使用weight_controller管理所有权重
// - 确认LayerNorm参数接口使用ln_param_valid
//
// 功能：
// 处理单个Transformer层的完整计算流程
// 
// 架构：
// 1. 层内存储：result_buffer_single_port (21KB) + ln1_result_buffer (1KB)
// 2. 计算模块：Attention(4-Head并行), FFN, Residual x2, LayerNorm x2
// 3. 控制器：transformer_block_ctrl_fsm
//
// 数据流：
// Layer Token Buffer(外部) → Attention → Result Buffer
//                          → Residual 1 → Result Buffer
//                          → LayerNorm 1 → Result Buffer + LN1 Result Buffer(内部)
//                          → FFN → Result Buffer
//                          → Residual 2 → Result Buffer (从LN1 Result Buffer读)
//                          → LayerNorm 2 → Layer Token Buffer(外部)
//
// 权重管理：
// 所有权重（QKV, WO, FFN, LayerNorm）统一由weight_controller管理
//
// 更新日志：
// 2024-11-16: v2.2 - 统一weight_controller接口
// 2024-11-16: v2.1 - 添加独立的LN1 Result Buffer
// 2024-11-15: v2.0 - 初始版本
//================================================================================

module backbone_transformer #(
    // ========== 网络参数 ==========
    ...
    //================================================================================
    // FFN权重接口（数组格式 - 连接到weight_controller）
    //================================================================================
...
initial begin
    $display("========================================");
    $display("Backbone Transformer v2.2 (Fixed)");
    $display("========================================");
    $display("Updates:");
    $display("  ✓ Unified weight_controller interface");
    $display("  ✓ FFN uses weight request (no DRAM)");
    $display("  ✓ FFN weight array: 32 independent exponents");
    $display("  ✓ LayerNorm param interface: ln_param_valid");
    $display("  ✓ Parallel Attention (4-Head)");
    $display("  ✓ QKV/WO array format (32 exponents)");
