...
//================================================================================
// QKV计算引擎 v3.0 (高性能 + 稳健握手版)
// 
// 配置目标：
// - 总计算能力：32 INT8 元素 (2个PE x 16元素)
// - 打包器：匹配 data_packer_PE_A (128-bit 输入)
// - 修复：增加握手确认状态，解决 System Stuck 死锁
//================================================================================
...
module qkv_compute_engine #(
    ...
    localparam STATE_WAIT_ACK       = 4'hB; // 【新增】握手确认状态
    ...
    compute_engine #(
        ...
        // 【关键恢复】使用 Type 0 (INT8x16) 来处理 16 个元素
        .PE_TYPE_0(0), 
        .PE_TYPE_1(0), // PE1 也处理 16 个元素
        
        .EXP_WIDTH(EXP_WIDTH), 
        .INPUT_MANT_WIDTH(MANT_WIDTH),
        
        // 【关键恢复】每个 PE 处理 16 个元素 -> 总共 32
        .ELEM_PE0(16),  
        .ELEM_PE1(16),  
        .TOTAL_ELEM(TOTAL_ELEM), // 32
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                STATE_WAIT_ACK: begin
                     // 此时 CE 已经捕获了 Valid=1，我们撤销 Valid
                     ce_input_valid <= 1'b0; 
                     state <= STATE_WAIT_CE; 
                end
                
                STATE_WAIT_CE: begin
                    ce_input_valid <= 1'b0; // 确保低电平
                    if (&collected_mask) begin
                        ce_result_ready <= 1'b0;
